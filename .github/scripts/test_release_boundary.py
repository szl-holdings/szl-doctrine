...
class BoundaryTests(unittest.TestCase):
    ...
    def test_python_identities_bind_source_and_target_constant_groups(self) -> None:
        ...
        ordinary_reads = valid + (
            ...
            b"dict.__setitem__(safe_settings, 'mode', 'safe')\n"
            b"operator.setitem(safe_settings, 'tone', 'calm')\n"
            b"safe_settings.__ior__({'theme': 'light'})\n"
            b"safe_settings |= {'layout': 'compact'}\n"
        ...
        bypasses = [
            ...
            valid + b'builtins_module = sys.modules["builtins"]\nrunner = builtins_module.exec\n',
            ...
            valid + b'namespace = sys._getframe().f_builtins\n',
            valid + b'import warnings\nkey = "HF_" + "REPO"\nwarnings.__builtins__[key] = "wrong"\n',
            valid + b'import importlib\nloader = importlib.import_module\nbuiltins_module = loader("builtins")\nrunner = builtins_module.exec\nrunner("HF_REPO = 1")\n',
            valid + b'import importlib.machinery\nfinder = importlib.machinery.BuiltinImporter.find_spec\nspec = finder("builtins")\ncreator = importlib.machinery.BuiltinImporter.create_module\nbuiltins_module = creator(spec)\nrunner = builtins_module.exec\n',
            valid + b'from importlib.machinery import BuiltinImporter as Loader\nfinder = Loader.find_spec\n',
            valid + b'import pickle\npayload = b"callback payload"\ndeserialize = pickle.loads\ndeserialize(payload)\n',
            valid + b'from pickle import loads as deserialize\npayload = b"callback payload"\ndeserialize(payload)\n',
            ...
            valid + b'key = "HF_" + "REPO"\nglobals()[key] = "wrong"\n',
            valid + b'key = "HF_" + "REPO"\ndel globals()[key]\n',
            valid + b'key = "HF_" + "REPO"\ndict.__setitem__(sys.modules["__main__"].__dict__, key, "wrong")\n',
            valid + b'key = "HF_" + "REPO"\ndict.__delitem__(sys.modules["__main__"].__dict__, key)\n',
            valid + b'key = "HF_" + "REPO"\noperator.setitem(sys.modules["__main__"].__dict__, key, "wrong")\n',
            valid + b'key = "HF_" + "REPO"\ndict.update(sys.modules["__main__"].__dict__, {key: "wrong"})\n',
            valid + b'key = "HF_" + "REPO"\ndict.clear(sys.modules["__main__"].__dict__)\n',
            valid + b'mutator = dict.__setitem__\nnamespace = sys.modules["__main__"].__dict__\nmutator(namespace, "HF_REPO", "wrong")\n',
            valid + b'def mutate(receiver, key, value):\n    receiver.update({key: value})\nnamespace = sys.modules["__main__"].__dict__\nmutate(namespace, "HF_REPO", "wrong")\n',
            valid + b'key = "HF_" + "REPO"\nsys.modules["__main__"].__dict__.__ior__({key: "wrong"})\n',
            valid + b'key = "HF_" + "REPO"\ndict.__ior__(sys.modules["__main__"].__dict__, {key: "wrong"})\n',
            valid + b'key = "HF_" + "REPO"\noperator.ior(sys.modules["__main__"].__dict__, {key: "wrong"})\n',
            valid + b'namespace = sys.modules["__main__"].__dict__\nnamespace |= {"HF_REPO": "wrong"}\n',
            valid + b'mutate = dict.__ior__\nnamespace = sys.modules["__main__"].__dict__\nmutate(namespace, {"HF_REPO": "wrong"})\n',
